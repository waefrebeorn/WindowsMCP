...
class ConsoleFormatter:
    ...
    def print_gemini(text: str):  # Used for non-streamed full messages or errors
        console.print(
            Panel(
                Text(text, style="purple"),
                title="[bold purple]Gemini[/bold purple]",
                border_style="purple",
            )
        )
    ...
    def print_gemini_header():
        console.print(
            Text("Gemini:", style="bold purple"), end=" "
        )  # No panel for streaming start
    ...
    def print_thought(text: str):
        ...
        # If needed, could be:
        # console.print(Panel(Text(text, style="yellow"),
    ...
    def print_tool_result(result: Any):
        try:
            ...
            TypeError
        ):  # Handle cases where result is not directly JSON serializable (e.g. already a string)
    ...
    def print_provider_response_header(provider_name: str):
        ...
        if provider_name.lower() == "ollama":
            ...
            style = "purple"
        console.print(
            Text(f"{provider_name.capitalize()}:", style=f"bold {style}"), end=" "
        )
    ...
    def print_provider_message(provider_name: str, text: str):
        ...
        console.print(
            Panel(
                ...
                title=f"[{title_style}]{provider_name.capitalize()}[/{title_style}]",
                border_style=style,
            )
        )
    ...
    def print_provider_error(provider_name: str, text: str):
        ...
        console.print(
            Panel(
                Text(text, style=style),
                title=f"[{title_style}]{provider_name.capitalize()} Error[/{title_style}]",
                border_style=style,
            )
        )
...
if __name__ == "__main__":
    ...
    ConsoleFormatter.print_provider_message(
        "Gemini", "This is a test Gemini message via generic method."
    )
    ...
    ConsoleFormatter.print_provider_message(
        "Ollama", "This is a test Ollama message via generic method."
    )
    ...
    ConsoleFormatter.print_tool_error(
        {"error_code": 500, "message": "Something went wrong."}
    )
