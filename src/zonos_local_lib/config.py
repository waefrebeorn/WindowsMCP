...
class ZonosConfig:
    ...
    def from_dict(cls, d: dict) -> "ZonosConfig":
        d_copy = d.copy() # Use d_copy to avoid modifying original dict
        backbone_config = BackboneConfig(**d_copy.pop("backbone"))
        prefix_conditioner_config = PrefixConditionerConfig(**d_copy.pop("prefix_conditioner"))
        config = cls(backbone_config, prefix_conditioner_config, **d_copy)
        return config
