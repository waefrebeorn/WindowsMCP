...
def load_whisper_model(model_name: str = DEFAULT_WHISPER_MODEL):
    ...
    if model_name not in _whisper_model_cache:
        ...
        try:
            ...
            logger.error(
                f"Failed to load Whisper model '{model_name}': {e}", exc_info=True
            )
...
def list_available_audio_devices() -> list:
    """Returns a list of available audio input devices and their properties."""
    try:
        devices = sd.query_devices()
        input_devices = []
        if isinstance(devices, dict):  # Handles case where only one device is found
            if devices.get("max_input_channels", 0) > 0:
                input_devices.append(
                    {
                        "id": devices.get(
                            "index", 0
                        ),  # query_devices might not return index directly for single device
                        "name": devices.get("name"),
                        "hostapi": devices.get("hostapi"),
                        "max_input_channels": devices.get("max_input_channels"),
                        "default_samplerate": devices.get("default_samplerate"),
                    }
                )
            ...
            for i, device_info in enumerate(devices):
                if device_info.get("max_input_channels", 0) > 0:
                    input_devices.append(
                        {
                            "id": i,  # Use enumeration index as ID if not in device_info
        ...
        logger.info(f"Found input audio devices: {input_devices}")
        return input_devices
    except Exception as e:
        logger.error(f"Could not query audio devices: {e}", exc_info=True)
        return []
...
def record_audio(
    ...
    """
    Records audio from the specified (or default) microphone for a
    specified duration and saves it as a temporary WAV file.

    Args:
        duration_seconds: The duration of the recording in seconds.
        sample_rate: The sample rate for the recording.
        channels: The number of audio channels.
        device: Optional. The ID (int) or name (str) of the input device.
    ...
    if device:
        logger.info(
            f"Attempting to record audio for {duration_seconds} seconds from device: {device}..."
        )
    else:
        logger.info(
            f"Starting audio recording for {duration_seconds} seconds from default device..."
        )
    ...
    try:
        ...
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".wav", prefix="user_audio_"
        )
        ...
        logger.error(
            "This might indicate an issue with your microphone setup or sounddevice/PortAudio installation."
        )
        logger.error(
            "Common issues: No default microphone, microphone muted, or driver problems."
        )
    except Exception as e:
...
def transcribe_audio_with_whisper(
    audio_file_path: str, model_name: str = DEFAULT_WHISPER_MODEL
) -> Optional[str]:
    """
    ...
    logger.info(
        f"Transcribing audio file: {audio_file_path} using Whisper model: {model_name}"
    )
    try:
        model = load_whisper_model(model_name)
        if model is None:  # Should not happen if load_whisper_model raises on failure
            logger.error(
                f"Whisper model '{model_name}' could not be loaded for transcription."
            )
            return None
        ...
        if (
            audio_file_path
            and "user_audio_" in audio_file_path
            and audio_file_path.endswith(".wav")
        ):
            try:
                ...
                logger.error(
                    f"Failed to clean up temporary audio file {audio_file_path}: {e}"
                )
    return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            "Please ensure Whisper is installed correctly and model files are accessible."
        )
        logger.error("Try: pip install openai-whisper")
        logger.error(
            "You might also need ffmpeg: sudo apt-get install ffmpeg (Linux) or choco install ffmpeg (Windows)"
        )
    ...
    if temp_audio_file:
        ...
        transcribed_text = transcribe_audio_with_whisper(
            temp_audio_file, model_name=DEFAULT_WHISPER_MODEL
        )
