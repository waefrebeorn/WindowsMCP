...
from google.genai import (
    types,
)  # I.2, III.1. types.Part will be used. ToolOutput removed.
...
from desktop_tools.desktop_tools_definitions import (
    DESKTOP_TOOLS_INSTANCE,
    get_ollama_tools_json_schema as get_desktop_ollama_schema,
)
from desktop_tools.tool_dispatcher import (
    DesktopToolDispatcher,
    FunctionCall,
)  # Assuming FunctionCall is also in tool_dispatcher or a shared types module
# from rich.status import Status # This was unused
...
parser.add_argument(
    "--test_command", type=str, help="Execute a single test command and exit."
)
parser.add_argument(
    "--test_file",
    type=str,
    help="Path to a file containing a list of test commands, one per line.",
)
...
OLLAMA_MODEL_NAME = (
    args.ollama_model
)  # This now comes from args, falling back to config via argparse default
...
if LLM_PROVIDER == "gemini":
    ...
    if ENV_GEMINI_API_KEY:
        ...
        and CONFIG_GEMINI_API_KEY != "None"
        and CONFIG_GEMINI_API_KEY is not None
    ):
        ...
        console.print(
            Panel(
                "[bold yellow]Warning:[/bold yellow] GEMINI_API_KEY not found for Gemini provider. Using a dummy key 'DUMMY_KEY'. Gemini calls will likely fail.",
                title="[yellow]Config Warning[/yellow]",
            )
        )
    ...
    GEMINI_MODEL_NAME = (
        ...
        or config.get("GEMINI_MODEL_NAME")
        or DEFAULT_CONFIG["GEMINI_MODEL_NAME"]
    )
    logger.info(f"Using Gemini Model: {GEMINI_MODEL_NAME}")
elif LLM_PROVIDER == "ollama":
    logger.info(
        f"Using Ollama provider with API URL: {OLLAMA_API_URL} and Model: {OLLAMA_MODEL_NAME}"
    )
...
history_file_path_str = config.get(
    "HISTORY_FILE_PATH", DEFAULT_CONFIG["HISTORY_FILE_PATH"]
)
...
try:
    ...
    console.print(
        Panel(
            f"[yellow]Warning: Could not create directory for history file '{history_file}': {e}[/yellow]",
            title="[yellow]File History Warning[/yellow]",
        )
    )
...
async def _process_command(
    ...
    try:
        ...
        while current_retry_attempt < MAX_API_RETRIES:
            try:
                if current_retry_attempt > 0:
                    ...
                    with console.status(
                        f"[bold yellow]{delay_message_provider} API error. Retrying in {current_delay:.1f}s (Attempt {current_retry_attempt + 1}/{MAX_API_RETRIES})...[/bold yellow]",
                        spinner="dots",
                    ):
                ...
                if (
                    llm_provider == "gemini"
                ):  # Check provider again here for provider-specific error handling
                    logger.warning(
                        f"Gemini API ServerError (Attempt {current_retry_attempt + 1}/{MAX_API_RETRIES}): {e}"
                    )
                    current_retry_attempt += 1
                    if current_retry_attempt >= MAX_API_RETRIES:
                        logger.error(
                            f"Max retries reached for Gemini API call. Last error: {e}"
                        )
                        ConsoleFormatter.print_provider_error(
                            "Gemini",
                            f"I encountered a persistent server error after {MAX_API_RETRIES} attempts: {e.message or str(e)}",
                        )
                    ...
                    logger.error(
                        f"Unexpected ServerError with {llm_provider}: {e}",
                        exc_info=True,
                    )
                    ConsoleFormatter.print_provider_error(
                        llm_provider, f"An unexpected server error occurred: {str(e)}"
                    )
                    command_processed_successfully = False
                    break
            except asyncio.TimeoutError as e:
                logger.warning(
                    f"{llm_provider.capitalize()} API TimeoutError (Attempt {current_retry_attempt + 1}/{MAX_API_RETRIES}): {e}"
                )
                current_retry_attempt += 1
                if current_retry_attempt >= MAX_API_RETRIES:
                    logger.error(
                        f"Max retries reached for {llm_provider.capitalize()} API call due to timeout. Last error: {e}"
                    )
                    ConsoleFormatter.print_provider_error(
                        llm_provider,
                        f"The request timed out after {MAX_API_RETRIES} attempts.",
                    )
                    command_processed_successfully = False
                    break
                current_delay *= RETRY_BACKOFF_FACTOR
                ...
                logger.error(
                    f"Unexpected error during {llm_provider.capitalize()} API call (Attempt {current_retry_attempt + 1}): {e}",
                    exc_info=True,
                )
                error_message = str(e)
                if llm_provider == "ollama" and "Connection refused" in error_message:
                    ...
                    ConsoleFormatter.print_provider_error(
                        llm_provider,
                        f"I encountered an unexpected error: {error_message}",
                    )
                command_processed_successfully = False
        ...
        if response is None:  # Failed to get initial response
            if is_test_file_command:
                console.print(
                    f"[bold red]Skipping processing for command '{user_input_str}' due to API failure with {llm_provider}.[/bold red]"
                )
        ...
        while True:  # Loop for iterative tool calls
            ...
            if llm_provider == "gemini":
                ...
                if response and response.get("message"):
                    ...
                    if assistant_message.get("tool_calls"):
                        logger.info(
                            f"Ollama response contains tool_calls: {assistant_message['tool_calls']}"
                        )
                        for ollama_tc in assistant_message["tool_calls"]:
                            if ollama_tc.get("function") and ollama_tc["function"].get(
                                "name"
                            ):
                                fc_id = ollama_tc.get(
                                    "id"
                                ...
                                if not fc_id:
                                    logger.warning(
                                        f"Ollama tool_call for '{fc_name}' is missing an ID. Generating one."
                                    )
                                    fc_id = uuid.uuid4().hex
                                fc_args_str = ollama_tc["function"].get(
                                    "arguments", "{}"
                                )  # Arguments are often a string
                                fc_args = {}
                                try:
                                    fc_args = json.loads(fc_args_str)
                                except json.JSONDecodeError:
                                    logger.error(
                                        f"Ollama tool call arguments from 'tool_calls' for ID {fc_id} are not valid JSON: {fc_args_str}"
                                    )
                                ...
                                pending_function_calls.append(
                                    FunctionCall(id=fc_id, name=fc_name, args=fc_args)
                                )  # Store ID
                                logger.info(
                                    f"Appended tool call from 'tool_calls': ID {fc_id}, Name {fc_name} with args {fc_args}"
                                )
                            else:
                                logger.warning(
                                    f"Ollama tool_call item in unexpected format (missing function/name): {ollama_tc}"
                                )
                        ...
                        if raw_content_str and isinstance(raw_content_str, str):
                            ...
                            logger.info(
                                f"Ollama response has content, attempting to parse. Initial content (stripped, snippet): {content_str_stripped[:200]}..."
                            )

                            # Phi4-mini specific parsing for "functools[...]"
                            if content_str_stripped.startswith(
                                "functools["
                            ) and content_str_stripped.endswith("]"):
                                logger.info(
                                    "Detected 'functools[]' wrapper in Ollama content."
                                )
                                json_list_str = content_str_stripped[
                                    len("functools[") : -1
                                ]
                                try:
                                    ...
                                    if isinstance(tool_call_list, list):
                                        logger.info(
                                            f"Successfully parsed 'functools[]' content as JSON list. Number of tool calls: {len(tool_call_list)}"
                                        )
                                        for tc_dict in tool_call_list:
                                            if (
                                                ...
                                                and (
                                                    "name" in tc_dict
                                                    or "function_name" in tc_dict
                                                )
                                                and "arguments" in tc_dict
                                            ):
                                                ...
                                                if not fc_name:
                                                    logger.warning(
                                                        f"Tool call from 'functools[]' list is missing 'name' or 'function_name'. Item: {tc_dict}. Skipping."
                                                    )
                                                    continue
                                                ...
                                                if isinstance(fc_args, str):
                                                    try:
                                                        ...
                                                        logger.error(
                                                            f"Failed to parse string 'arguments' from 'functools[]' tool call for '{fc_name}': {fc_args}. Error: {e_inner}. Skipping."
                                                        )
                                                        continue

                                                if not isinstance(fc_args, dict):
                                                    logger.warning(
                                                        f"Tool call from 'functools[]' for '{fc_name}' has 'arguments' not as dict or parsable string: {type(fc_args)}. Skipping."
                                                    )
                                                    continue

                                                tool_call_id = (
                                                    uuid.uuid4().hex
                                                )  # Generate ID as phi4-mini might not provide one here
                                                pending_function_calls.append(
                                                    FunctionCall(
                                                        id=tool_call_id,
                                                        name=fc_name,
                                                ...
                                                logger.info(
                                                    f"Appended tool call from 'functools[]' list with generated ID {tool_call_id}: {fc_name} with args {fc_args}"
                                                )
                                            else:
                                                logger.warning(
                                                    f"Item in 'functools[]' JSON list is not a valid tool call dict: {tc_dict}. Skipping."
                                                )
                                    else:
                                        logger.warning(
                                            f"'functools[]' content parsed, but not as a JSON list: {type(tool_call_list)}. Content: {json_list_str[:100]}. Falling back."
                                        )
                                        # Fall through to general JSON parsing
                                except json.JSONDecodeError as e:
                                    logger.error(
                                        f"Failed to parse JSON from 'functools[]' content: '{json_list_str}'. Error: {e}. Falling back."
                                    )
                                    # Fall through to general JSON parsing
                            ...
                            if (
                                not pending_function_calls
                            ):  # Only try this if the functools[] parsing didn't yield calls
                                json_to_parse = (
                                    content_str_stripped  # Use already stripped content
                                )
                                is_markdown_json = False
                                if json_to_parse.startswith(
                                    "```"
                                ) and json_to_parse.endswith("```"):
                                    is_markdown_json = True
                                    logger.info(
                                        "Markdown ``` detected at start and end (fallback path)."
                                    )
                                    json_to_parse = json_to_parse[3:-3].strip()
                                    if content_str_stripped.startswith("```json"):
                                        logger.info(
                                            "Potential 'json' prefix after ``` found (fallback path)."
                                        )
                                        if json_to_parse.lower().startswith("json"):
                                            json_to_parse = json_to_parse[4:].lstrip()
                                            logger.info(
                                                f"Removed 'json' prefix. String for parsing (fallback, snippet): {json_to_parse[:200]}..."
                                            )
                                        else:
                                            logger.info(
                                                f"Prefix was ```json but 'json' not at start of inner content (fallback): {json_to_parse[:10]}"
                                            )
                                    else:
                                        logger.info(
                                            f"Standard ``` block. String for parsing (fallback, snippet): {json_to_parse[:200]}..."
                                        )

                                if not json_to_parse:
                                    logger.info(
                                        "Content became empty after stripping potential Markdown markers (fallback path). Treating as text."
                                    )
                                else:
                                    try:
                                        ...
                                        if (
                                            ...
                                            and (
                                                "name" in potential_tool_call
                                                or "function_name"
                                                in potential_tool_call
                                            )
                                            and "arguments" in potential_tool_call
                                        ):

                                            fc_name = potential_tool_call.get(
                                                "name"
                                            ) or potential_tool_call.get(
                                                "function_name"
                                            )
                                            ...
                                            if not fc_name:
                                                logger.warning(
                                                    f"Tool call from content JSON (fallback) is missing a valid 'name' or 'function_name'. Dict: {potential_tool_call}. Skipping."
                                                )
                                            else:
                                                logger.info(
                                                    f"Identified potential single tool call from content (fallback): '{fc_name}'"
                                                )
                                                if fc_args is None:
                                                    fc_args = {}
                                                    ...
                                                    try:
                                                        ...
                                                        logger.error(
                                                            f"Failed to parse string 'arguments' (fallback) for '{fc_name}': {fc_args}. Error: {e_inner}. Skipping."
                                                        )
                                                        fc_name = (
                                                            None  # Prevent appending
                                                        )

                                                if fc_name and not isinstance(
                                                    fc_args, dict
                                                ):
                                                    logger.warning(
                                                        f"Tool call (fallback) for '{fc_name}' has 'arguments' not as dict/parsable string (and not None): {type(fc_args)}. Skipping."
                                                    )
                                                elif (
                                                    fc_name
                                                ):  # fc_name is still valid and fc_args is a dict
                                                    ...
                                                    logger.info(
                                                        f"Appended tool call from 'content' JSON (fallback) with ID {tool_call_id}: {fc_name} with args {fc_args}"
                                                    )
                                        else:
                                            logger.info(
                                                "Ollama content JSON (fallback, after potential Markdown stripping) does not match tool call structure. Treating as text."
                                            )
                                    except json.JSONDecodeError:
                                        if is_markdown_json:
                                            logger.error(
                                                f"Failed to parse JSON extracted from Markdown (fallback): '{json_to_parse}'"
                                            )
                                        else:
                                            logger.info(
                                                f"Ollama content is not JSON (fallback), treating as text. Snippet: {json_to_parse[:100]}"
                                            )
                                    except Exception as e:
                                        logger.error(
                                            f"Unexpected error parsing Ollama content as tool call (fallback): {e}",
                                            exc_info=True,
                                        )
                        else:
                            logger.info(
                                "Ollama message content is empty or not a string. No fallback tool call parsing."
                            )

                    if pending_function_calls:
                        logger.info(
                            f"Proceeding with {len(pending_function_calls)} pending function calls for Ollama."
                        )
                    else:
                        logger.info(
                            "No tool calls detected from Ollama response. Will print content if any."
                        )
            ...
            if not pending_function_calls:
                ...
                logger.info(
                    f"Consecutive tool call count: {consecutive_tool_calls_count}"
                )

                if consecutive_tool_calls_count > MAX_CONSECUTIVE_TOOL_CALLS:
                    logger.warning(
                        f"Ollama exceeded max consecutive tool calls ({MAX_CONSECUTIVE_TOOL_CALLS}). Intervening."
                    )
                    if llm_provider == "ollama":
                        ...
                        if isinstance(ollama_history, list):
                            ...
                            logger.info(
                                f"Sent intervention message to Ollama: {intervention_message_content}"
                            )
                            console.print(
                                Panel(
                                    "[bold yellow]Max consecutive tool calls reached. An intervention message has been sent to the assistant to encourage a direct response or clarification.[/bold yellow]",
                                    title="[orange_red1]Loop Intervention[/orange_red1]",
                                    expand=False,
                                )
                            )
                            ...
                            logger.error(
                                "Cannot send intervention message: ollama_history is not a list."
                            )
            ...
            tool_call_results = await asyncio.gather(
                *tool_tasks
            )  # Results are dicts: {'name': ..., 'response': ...}
            ...
            if llm_provider == "gemini":
                ...
                if not tool_response_parts:
                    logger.warning(
                        "No tool response parts to send for Gemini, though function calls were expected."
                    )
                ...
                while current_retry_attempt_tool < MAX_API_RETRIES:
                    try:
                        if current_retry_attempt_tool > 0:
                            with console.status(
                                f"[bold yellow]Gemini API error (tool response). Retrying in {current_delay_tool:.1f}s...[/bold yellow]",
                                spinner="dots",
                            ):
                                await asyncio.sleep(current_delay_tool)
                        with console.status(
                            f"[bold green]Gemini is processing tool results...[/bold green]",
                            spinner="dots",
                        ):
                        ...
                        logger.warning(
                            f"Gemini API ServerError (tool response) (Attempt {current_retry_attempt_tool + 1}/{MAX_API_RETRIES}): {e}"
                        )
                        ...
                        if current_retry_attempt_tool >= MAX_API_RETRIES:
                            ConsoleFormatter.print_provider_error(
                                "Gemini",
                                f"Max retries sending tool results: {e.message or str(e)}",
                            )
                            command_processed_successfully = False
                            break
                        ...
                        logger.warning(
                            f"Gemini API TimeoutError (tool response) (Attempt {current_retry_attempt_tool + 1}/{MAX_API_RETRIES}): {e}"
                        )
                        current_retry_attempt_tool += 1
                        if current_retry_attempt_tool >= MAX_API_RETRIES:
                            ConsoleFormatter.print_provider_error(
                                "Gemini",
                                f"Timeout sending tool results after {MAX_API_RETRIES} attempts.",
                            )
                            command_processed_successfully = False
                            break
                        current_delay_tool *= RETRY_BACKOFF_FACTOR
                        ...
                        logger.error(
                            f"Unexpected error Gemini API (tool response) (Attempt {current_retry_attempt_tool + 1}): {e}",
                            exc_info=True,
                        )
                        ConsoleFormatter.print_provider_error(
                            "Gemini", f"Unexpected error sending tool results: {str(e)}"
                        )
                        command_processed_successfully = False
                        break
                ...
                for result_dict in tool_call_results:
                    ...
                    if not tool_call_id_for_ollama:
                        logger.warning(
                            f"Tool result for '{result_dict.get('name')}' is missing an ID. Ollama might not be able to map this result correctly."
                        )
                    ...
                    if (
                        ...
                        and list(response_data.keys()) == ["content"]
                        and isinstance(response_data["content"], str)
                    ):
                        content_for_ollama = response_data["content"]
                        logger.info(
                            f"Extracted simple text content for Ollama tool result (ID: {tool_call_id_for_ollama}): '{content_for_ollama}'"
                        )
                    else:
                        content_for_ollama = json.dumps(response_data)
                        logger.info(
                            f"Using JSON dump for Ollama tool result (ID: {tool_call_id_for_ollama}): {content_for_ollama}"
                        )
                    ...
                    logger.info(
                        f"Appended tool result to Ollama history: ID {tool_call_id_for_ollama}, Name {result_dict.get('name')}"
                    )
                ...
                response_tool_call = None
                while (
                    current_retry_attempt_tool < MAX_API_RETRIES
                ):  # Retry loop for Ollama after tool call
                    try:
                        if current_retry_attempt_tool > 0:
                            with console.status(
                                f"[bold yellow]Ollama API error (tool response). Retrying in {current_delay_tool:.1f}s...[/bold yellow]",
                                spinner="dots",
                            ):
                                await asyncio.sleep(current_delay_tool)
                        with console.status(
                            f"[bold green]Ollama is processing tool results...[/bold green]",
                            spinner="dots",
                        ):
                            response_tool_call = await asyncio.to_thread(
                                ...
                                messages=ollama_history,
                                tools=(
                                    get_desktop_ollama_schema()
                                    if get_desktop_ollama_schema()
                                    else None
                                ),  # Resend tools if needed by Ollama
                            )
                        if response_tool_call and response_tool_call.get("message"):
                            ollama_history.append(
                                response_tool_call["message"]
                            )  # Add Ollama's new response to history
                        break  # Success
                    except (
                        asyncio.TimeoutError
                    ) as e:  # Specific Ollama timeout for tool response
                        logger.warning(
                            f"Ollama API TimeoutError (tool response) (Attempt {current_retry_attempt_tool + 1}/{MAX_API_RETRIES}): {e}"
                        )
                        current_retry_attempt_tool += 1
                        if current_retry_attempt_tool >= MAX_API_RETRIES:
                            ConsoleFormatter.print_provider_error(
                        ...
                        logger.error(
                            f"Ollama API error (tool response) (Attempt {current_retry_attempt_tool + 1}): {e}",
                            exc_info=True,
                        )
                        ConsoleFormatter.print_provider_error(
                            "Ollama", f"API error sending tool results: {str(e)}"
                        )
                        current_retry_attempt_tool += 1
                        if current_retry_attempt_tool >= MAX_API_RETRIES:
                            command_processed_successfully = False
                        ...
                        current_delay_tool *= RETRY_BACKOFF_FACTOR
                response = (
                    response_tool_call  # Update main response with Ollama's reply
                )

            if not command_processed_successfully or response is None:
                logger.warning(
                    f"Failed to get valid response from {llm_provider} after tool processing. Breaking from tool loop."
                )
        ...
        if llm_provider == "ollama" and intervention_occurred_this_turn:
            ...
            if (
                ...
                and response["message"].get("tool_calls")
            ):
                ...
                logger.warning(
                    "Ollama attempted further tool calls post-intervention. Command processing halted."
                )
                command_processed_successfully = (
                    False  # Consider this an unsuccessful command completion
                )
            ...
            if response and response.text:
                ...
                if response and response.text:
                    ...
                    for char_chunk in final_text_response:
                        ConsoleFormatter.print_provider_response_chunk(
                            "Gemini", char_chunk
                        )
                    console.print()
                elif (
                    ...
                    if final_text_response:
                        ...
                        for (
                            char_chunk
                        ) in final_text_response:  # Iterate over the joined string
                            ConsoleFormatter.print_provider_response_chunk(
                                "Gemini", char_chunk
                            )
                        console.print()
                    else:
                        ConsoleFormatter.print_provider_message(
                            "Gemini", "(No text content found in final response parts)"
                        )
                else:
                    ConsoleFormatter.print_provider_message(
                        "Gemini",
                        "(No text response or recognizable content from Gemini)",
                    )

                if final_text_response and config.get("ENABLE_TTS", True):
                    from desktop_tools import (
                        voice_output,
                    )  # Import late to avoid issues if not used
                ...
                if (
                    ...
                    ConsoleFormatter.print_provider_response_chunk(
                        "Ollama", final_text_response
                    )
                    console.print()
                elif intervention_occurred_this_turn:
                    ConsoleFormatter.print_provider_message(
                        "Ollama",
                        "(Intervention occurred, and Ollama did not provide a subsequent text response or tool call.)",
                    )
                else:
                    ConsoleFormatter.print_provider_message(
                        "Ollama", "(No text content in final response from Ollama)"
                    )
    ...
    #         command_processed_successfully = False
        ...
        logger.error(
            f"A request to {llm_provider} or a tool timed out for command: {user_input_str}."
        )
        console.print(
            Panel(
                f"[bold red]A request to {llm_provider} or a tool timed out. Please try again.[/bold red]",
        ...
        logger.error(
            f"An error occurred during the chat loop for command '{user_input_str}' with {llm_provider}: {e}",
            exc_info=True,
        )
        error_msg = getattr(e, "message", str(e))
        ConsoleFormatter.print_provider_error(
            llm_provider, f"I encountered an internal error: {error_msg}"
        )
        command_processed_successfully = False
...
async def main_loop():
    ...
    if LLM_PROVIDER == "gemini":
        try:
            ...
            )  # transport='async' is default for genai.Client
            ...
            chat_session = llm_client.aio.chats.create(  # Gemini chat session
                ...
                history=[
                    types.Content(
                        role="user", parts=[types.Part(text=system_instruction_text)]
                    ),
                    types.Content(
                        role="model",
                        parts=[
            ...
            logger.info(
                f"Gemini client and chat session initialized for model {GEMINI_MODEL_NAME}."
            )
        except Exception as e:
            logger.critical(f"Failed to initialize Gemini client: {e}", exc_info=True)
            console.print(
                Panel(
                    f"[bold red]Critical Error:[/bold red] Failed to initialize Gemini: {e}",
                    title="[red]LLM Init Error[/red]",
                )
            )
            return
    elif LLM_PROVIDER == "ollama":
        try:
            ...
            try:
                ...
                console.print(
                    Panel(
                        f"[bold red]Warning:[/bold red] Could not connect to Ollama server at '{OLLAMA_API_URL}'. "
                        f"Please ensure Ollama is running and accessible. Error: {e}",
                        title="[yellow]Ollama Connection Error[/yellow]",
                    )
                )
            ...
            ]  # This is the history for Ollama
            logger.info(
                f"Ollama client initialized. Target model: {OLLAMA_MODEL_NAME}. System prompt set."
            )
        except ImportError:
            logger.critical(
                "Ollama provider selected, but 'ollama' library is not installed. Please run: pip install ollama"
            )
            console.print(
                Panel(
                    "[bold red]Critical Error:[/bold red] Ollama library not found. Please install it with `pip install ollama`.",
            ...
            console.print(
                Panel(
                    f"[bold red]Critical Error:[/bold red] Failed to initialize Ollama: {e}",
                    title="[red]LLM Init Error[/red]",
                )
            )
            return
    ...
    try:
        ...
        # from rich.status import Status  # Unused import
        ...
        if LLM_PROVIDER == "gemini":
            ...
            active_model_display = (
                f"Ollama Model: {OLLAMA_MODEL_NAME} (via {OLLAMA_API_URL})"
            )

        console.print(
            Panel(
                f"[bold green]Desktop AI Assistant Initialized ({LLM_PROVIDER.capitalize()})[/bold green]",
        ...
        if args.test_file:
            ...
            try:
                ...
                console.print(
                    f"[info]Found {file_command_total} commands in the test file.[/info]"
                )
                ...
                for i, user_input_str in enumerate(commands):
                    ...
                    ):  # Optional delay between commands in test file
                        console.print(
                            f"[dim]Waiting for 5 seconds before next command...[/dim]"
                        )
                ...
                console.print(
                    f"[bold red]Error: Test file '{args.test_file}' not found.[/bold red]"
                )
            except Exception as e:
                logger.error(
                    f"Error processing test file '{args.test_file}': {e}", exc_info=True
                )
                console.print(
                    f"[bold red]An unexpected error occurred while processing the test file: {e}[/bold red]"
                )
            return
            ...
            console.print(
                f"Type your commands for Desktop Automation ({LLM_PROVIDER.capitalize()} backend), or 'exit' to quit.",
                style="dim",
            )
            while True:
                ...
                try:
                    if args.voice:
                        ...
                        console.print(
                            "[cyan]Listening for voice command (Ctrl+C to cancel recording)...[/cyan]"
                        )
                        # Use a default recording duration or make it configurable
                        RECORDING_DURATION = config.get("VOICE_RECORDING_DURATION", 5)
                        WHISPER_MODEL_NAME = config.get(
                            "WHISPER_MODEL_NAME", voice_input.DEFAULT_WHISPER_MODEL
                        )
                        INPUT_AUDIO_DEVICE = config.get(
                            "INPUT_AUDIO_DEVICE_NAME_OR_ID", None
                        )

                        if INPUT_AUDIO_DEVICE:
                            logger.info(
                                f"Using input audio device from config: {INPUT_AUDIO_DEVICE}"
                            )
                        else:
                            logger.info("Using default input audio device.")

                        temp_audio_file = await asyncio.to_thread(
                        ...
                        if temp_audio_file:
                            console.print(
                                f"[dim]Audio recorded to {temp_audio_file}, transcribing...[/dim]"
                            )
                            user_input_str = await asyncio.to_thread(
                                voice_input.transcribe_audio_with_whisper,
                                temp_audio_file,
                                WHISPER_MODEL_NAME,
                            )
                            if user_input_str:
                            ...
                            console.print(
                                "[yellow]Audio recording failed. Please try again or type your command.[/yellow]"
                            )
                            continue  # Skip processing if recording failed
                    else:  # Standard text input
                        prompt_text = HTML(
                            f"<ansiblue><b>You ({LLM_PROVIDER.capitalize()}): </b></ansiblue>"
                        )
                        user_input_str = await asyncio.to_thread(
                            session.prompt, prompt_text, reserve_space_for_menu=0
                        )
                    ...
                    if args.voice:
                        ...
                        console.print(
                            "\n[bold yellow]Exiting assistant...[/bold yellow]"
                        )
                        break
                except EOFError:
                    console.print(
                        "\n[bold yellow]Exiting assistant (EOF)...[/bold yellow]"
                    )
                    break
    ...
    # MCPConnectionError is no longer relevant.
        ...
        logger.critical(
            f"Critical unhandled exception in main_loop: {e}", exc_info=True
        )
        console.print(
            Panel(
                f"[bold red]Critical unhandled exception:[/bold red] {e}",
        ...
        logger.info(
            f"Desktop Assistant application finished (LLM Provider: {LLM_PROVIDER})."
        )
        console.print(
            f"[bold yellow]Desktop Assistant application finished (LLM Provider: {LLM_PROVIDER}).[/bold yellow]"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if (
            str(e) not in ["0", "1"] and e.code is not None
        ):  # Don't print for clean exits like sys.exit(0) or sys.exit(1) without a message
